...
public class Game implements GameLogic, GameVisualization {
   ...
   public void drawStartScreen(int difficulty) {
    ...
    public void drawGameField(String playerType) {
        int x = 150;
        AtomicInteger y = new AtomicInteger(playerType.equals("Player") ? 80 : 305);
        turtle.textSize = 20;
        turtle.left(90)
              .moveTo(220, y.get())
              .color(50, 205, 50)
              .text(playerType.equals("Player") ? "Player" : "Computer")
              .right(90)
              .color(0, 0, 255);
        
        y.addAndGet(25);
        turtle.moveTo(x, y.get());
        IntStream.range(0, 10).forEach(i -> {
            ...
            y.addAndGet(15);
            turtle.penUp().moveTo(x, y.get()).penDown();
        });
        turtle.color(0, 0, 0);
    }

    private void drawSquare() {
        IntStream.range(0, 4).forEach(k -> turtle.forward(15).right(90));
    }
    ...
    public boolean placeShip(char[] field, int startX, int startY, int length, boolean horizontal) {
        if ((horizontal && startY + length > 10) || (!horizontal && startX + length > 10)) {
            return false;
        }
        boolean isSpaceFree = IntStream.range(0, length)
            .allMatch(i -> field[toIndex(startX + (horizontal ? 0 : i), startY + (horizontal ? i : 0))] == '~');
    ...
    public void checkAndMarkDestroyedShipsAI() {
        Iterator<Ship> iterator = aiShips.iterator();
    ...
    public void checkAndMarkDestroyedShipsPlayer() {
        Iterator<Ship> iterator = playerShips.iterator();
        while (iterator.hasNext()) {
            Ship ship = iterator.next();
    ...
    private void clearHitsOnShip(Ship ship) {
        int startRow = ship.row(), startCol = ship.column();
        if (ship.horizontal()) {
            IntStream.range(startCol, startCol + ship.length())
                    .forEach(i -> removeHitFromList(ai.lastHits, startRow, i));
        } else {
            IntStream.range(startRow, startRow + ship.length())
                    .forEach(i -> removeHitFromList(ai.lastHits, i, startCol));
        }
    }
    ...
    private int[] getFieldCoordinates(String field) {
        return field.equals("AI") ? new int[]{150, 330} : new int[]{150, 105};
    }

    public void drawHit(int row, int column, String field) {
        int[] baseCoordinates = getFieldCoordinates(field);
        int xStart = baseCoordinates[0] + (column * 15);
        int yStart = baseCoordinates[1] + (row * 15);
    ...
    public void drawRedLine(Ship ship, String field) {
        int[] baseCoordinates = getFieldCoordinates(field);
        double x = baseCoordinates[0];
        double y = baseCoordinates[1];
    ...
    private void declareWinner(String message, int red, int green, int blue) {
        turtle.textSize = 30;
        turtle.left(90).moveTo(250, 520).color(red, green, blue).text(message).right(90);
    ...
    private void resetTurtleAndDrawFields() {
        turtle.reset();
        drawGameField("Player");
        drawGameField("AI");
    }
...
public class AIMedium {
    ...
    public void makeAIShot(int row, int col) {
        int index = game.toIndex(row, col);
        char symbol = game.playerField[index];

        if (symbol == 'S') { 
            System.out.println("KI Treffer bei (" + row + ", " + col + ")");
            handleHit(row, col, index);
        } else {
            System.out.println("KI Fehlschuss bei (" + row + ", " + col + ")");
            handleMiss(row, col, index);
        }
    }

    public void handleHit(int row, int col, int index) {
        visibleFieldAI[index] = 'X';
        game.playerField[index] = 'X';
    ...
    public void handleMiss(int row, int col, int index) {
        visibleFieldAI[index] = 'O';
        game.playerField[index] = 'O';
...
class AIDifficult extends AIMedium {
    ...
    public boolean canPlaceWithoutNeighbors(int xStart, int yStart, int length, boolean horizontal) {
        if (horizontal && yStart + length > 10 || !horizontal && xStart + length > 10) {
            return false;
        }
        ...
        return IntStream.range(xMin, xMax + 1)
                        .boxed()
                        .flatMap(x -> IntStream.range(yMin, yMax + 1)
                                            .mapToObj(y -> new int[]{x, y}))
                        .noneMatch(coord -> game.aiField[game.toIndex(coord[0], coord[1])] != '~');
    }
    ...
    public boolean canShipHere(int row, int col) {
        ...
        return canShipFit(Arrays.asList(counterHorizontal, counterVertical));
    }
...
Dank dieser Methoden kann das Spiel zu Jederzeit neu gestartet werden oder beendet werden: Das heisst im Laufe des Spiels oder am Ende des Spiel . Um neu starten zu können werden fast alle Attribute von Klassen einfach wie im Konstruktoren initialisiert, nachdem ich das Turtle ganz reset() habe. Und ich rufe dann die Methode **setDifficulty(difficulty)** und **drawStartScreen(difficulty)** auf.
